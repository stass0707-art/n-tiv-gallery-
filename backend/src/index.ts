...
import { createSession, deleteSession, getSessionUser, hashPassword, requireAuth, requireAdmin } from './auth.js';
...
app.use(cors({
  origin: process.env.NODE_ENV === 'production' ? ['https://n-tiv.ru', 'https://www.n-tiv.ru'] : true,
  credentials: true,
}));
...
app.post('/api/auth/login', authLimiter, async (req: Request, res: Response) => {
  ...
  const userRes = await pool.query('SELECT id, email, password_hash, role FROM admin_users WHERE email = $1', [email]);
...
app.get('/api/pages/:slug', async (req: Request, res: Response) => {
  const result = await pool.query('SELECT * FROM pages WHERE slug = $1 AND published = true', [req.params.slug]);
  if (!result.rows.length) {
    res.status(404).json({ error: 'Not found' });
    return;
  }
...
app.get('/api/texts', async (_req: Request, res: Response) => {
  ...
  result.rows.forEach((row) => { texts[row.key] = row.value; });
  res.json(texts);
});
...
app.get('/api/admin/users', requireAuth, requireAdmin, async (_req: Request, res: Response) => {
  const result = await pool.query('SELECT id, email, role, created_at FROM admin_users ORDER BY created_at DESC');
  res.json(result.rows);
});
